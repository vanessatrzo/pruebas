...
;(function(){
  ...
  ClockPicker.prototype.show = function(e){
    ...
    var trimedValue = ((this.input.prop('value') || this.options['default'] || '') + '').replace(/^\s\s*/, '').replace(/\s\s*$/, '');
    ...
    if (trimedValue === 'now') {
      ...
      var match = trimedValue.toUpperCase().match(/^(\d{1,2}):(\d{1,2})\s*(AM|PM)?$/);
      if(match) {
        this.hours = +match[1];
        this.minutes = +match[2];
        this.amOrPm = match[3];
      } else {
        this.hours = 0;
        this.minutes = 0;
